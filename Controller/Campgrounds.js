...
module.exports.newCampground=async (req, res)=>{ 
    ...
    campground.images=req.files.map((obj)=>{return {url:obj.path, filename:obj.filename}})//looping and creating an array of object in images having url and filename of image file
...
module.exports.editCampground=async (req, res)=>{
    ...
    const imageObject= req.files.map((obj)=>{return {url:obj.path, filename:obj.filename}})
    ...
    campground.updateOne({$pull:{images:{filename:{$in:req.body.deleteImages}}}}) //this remove campground image from mongo server //pull or remove images having filename is in req.body.deleteCampground array
    }
    await campground.save();
